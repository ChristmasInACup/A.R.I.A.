...
public sealed class FirstSliceTests
{
    ...
    [Fact] public void Multi_level_delegation_rejects_wrong_ancestor_domain() { var root = Authority(domain: new DomainId("other"), scope: ResourceScope.For("case-a"), mayDelegate: true); var middle = Authority(scope: ResourceScope.For("case-a"), delegator: root, mayDelegate: true); var leaf = Authority(delegator: middle); var result = Processor(authorities: [leaf]).Process(Request(), "Review."); Assert.False(result.IsAllowed); Assert.Null(result.Proposal); }
    [Fact] public void Multi_level_delegation_rejects_wrong_ancestor_purpose() { var root = Authority(purpose: "export", scope: ResourceScope.For("case-a"), mayDelegate: true); var middle = Authority(scope: ResourceScope.For("case-a"), delegator: root, mayDelegate: true); var leaf = Authority(delegator: middle); var result = Processor(authorities: [leaf]).Process(Request(), "Review."); Assert.False(result.IsAllowed); Assert.Null(result.Proposal); }
